...
(function() {
	...
	function calculateEndPoint(latlng, dist, degree) {
		 ...
		 * a = arccos(cos(b)*cos(90 - lat1) + sin(90 - lat1)*sin(b)*cos(azimuth))—basically taking the arc cosine of the 
		 * Law of Cosines for a. From a, we can get lat2, so the only item remaining is to figure lon2; 
		...
		bearing =  degree * d2r;
		distance =  distance / R;
		var a = Math.acos(Math.cos(distance) * Math.cos((90 - latlng.lat)*d2r) + Math.sin((90 - latlng.lat)*d2r) * Math.sin(distance) * Math.cos(bearing));
	...
	function calculateArrowArray (latlng, degree) {
		...
		var firstEdge = calculateEndPoint(latlng, config.arrowheadLength, degree - config.arrowheadDegree);
		var arr = [firstEdge, latlng, calculateEndPoint(latlng, config.arrowheadLength, degree + config.arrowheadDegree)];
	...
	return {	
		...
		getStretchFactor : function () { return config.stretchFactor; },
